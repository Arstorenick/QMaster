  ...
  <div class="dept-page">
    <div class="dept-layout">
      ...
      <aside class="dept-sidebar">
        ...
        <input v-model="search" class="input" placeholder="搜索部门..." style="margin-bottom:12px" />
        <div class="dept-tree" v-if="filteredTree.length">
          <div v-for="dept in filteredTree" :key="dept.id">
            <DeptNode :dept="dept" :level="0" :active-id="activeDept?.id" :search="search" @select="selectDept" @delete="handleDelete" />
          </div>
        </div>
          ...
          <div class="card" style="padding:var(--spacing-xl);max-width:600px">
            ...
            <div class="form-group">
              ...
              <p class="text-secondary text-sm" style="margin-bottom:8px">
                <template v-if="importType === 'department'">格式：部门名,上级部门完整路径（可选）</template>
                <template v-else>格式：用户名,密码,角色(2/3/4),部门完整路径（可选）</template>
              </p>
              <textarea v-model="csvText" class="input" rows="8" placeholder="沈阳公司,
和平区分公司,沈阳公司
铁西区分公司,沈阳公司"></textarea>
            </div>
            <div class="form-group">
              <input type="file" accept=".csv" @change="handleFileUpload" class="input" />
            </div>
            <button class="btn btn-primary" @click="doImport" :disabled="!csvText.trim() || importing">
              {{ importing ? '导入中...' : '开始导入' }}
            </button>
            <div v-if="importResult" class="mt-md">
              <p class="text-sm" style="color:var(--color-success)">成功导入 {{ importResult.total }} 条</p>
              <p v-if="importResult.errors?.length" class="text-sm" style="color:var(--color-danger)">
        ...
        <template v-else-if="activeTab === 'dashboard'">
          ...
          <div class="dept-stats" style="margin-bottom:24px">
            <div class="stat-item">
              <span class="stat-num">{{ dashboard.total_users }}</span>
              <span class="stat-label">总用户数</span>
            </div>
            <div class="stat-item">
              <span class="stat-num">{{ dashboard.total_surveys }}</span>
              <span class="stat-label">总问卷数</span>
            </div>
            <div class="stat-item">
              <span class="stat-num">{{ dashboard.total_submissions }}</span>
          ...
          <div class="card" style="padding:var(--spacing-lg)">
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="d in dashboard.departments" :key="d.id">
                  <td><strong>{{ d.name }}</strong></td>
                  <td><span class="tag tag-primary">{{ d.member_count }} 人</span></td>
                  <td>{{ d.submission_count }} 份</td>
        ...
        <template v-else>
          <template v-if="activeDept">
            <div class="dept-detail-header">
              <div>
                <div class="flex items-center gap-sm">
                  ...
                  <span class="tag tag-primary">{{ activeDept.member_count }} 人</span>
                </div>
                <p class="text-secondary text-sm mt-sm">{{ activeDept.full_path }}</p>
              </div>
              <div class="flex items-center gap-sm">
                <button class="btn btn-secondary btn-sm" @click="activeTab = 'dashboard'">看板</button>
                <button class="btn btn-secondary btn-sm" @click="exportMembers">导出CSV</button>
              </div>
            </div>

            <div class="dept-stats">
              <div class="stat-item"><span class="stat-num">{{ activeDept.member_count }}</span><span class="stat-label">本级成员</span></div>
              <div class="stat-item"><span class="stat-num">{{ subDeptCount(activeDept) }}</span><span class="stat-label">下级部门</span></div>
              <div class="stat-item"><span class="stat-num">{{ activeDept.children?.length || 0 }}</span><span class="stat-label">直属子部门</span></div>
            </div>
            ...
            <div class="card" style="padding:var(--spacing-lg)">
              ...
              <table class="table" v-if="filteredMembers.length">
                <thead><tr><th>用户名</th><th>角色</th><th>邮箱</th><th>加入时间</th></tr></thead>
                <tbody>
          ...
          <div v-else class="dept-empty text-center">
            ...
            <h3>选择左侧部门查看</h3>
            <button class="btn btn-secondary btn-sm mt-md" @click="activeTab = 'dashboard'">查看统计看板</button>
          </div>
        </template>
      </section>
    </div>
    ...
    <Teleport to="body">
      <div class="modal-overlay" v-if="showCreate" @click.self="showCreate = false">
        <div class="modal card">
          <h3>新建部门</h3>
          <div class="form-group"><label>部门名称</label><input v-model="newName" class="input" placeholder="输入部门名称" /></div>
          <div class="form-group"><label>上级部门</label><select v-model="newParent" class="input"><option :value="null">无（顶级部门）</option><option v-for="d in flatDepts" :key="d.id" :value="d.id">{{ d.full_path }}</option></select></div>
...
async function loadData() {
  const [treeRes, flatRes, dashRes] = await Promise.all([
    departmentsAPI.tree(), departmentsAPI.flat(), departmentsAPI.dashboard().catch(() => ({ data: dashboard.value })),
  ])
...
async function handleDelete(id) {
  if (!confirm('确定删除此部门及其所有子部门？')) return
  await departmentsAPI.delete(id)
  if (activeDept.value?.id === id) activeDept.value = null
  await loadData()
}

function subDeptCount(dept) {
  let count = 0
  function countChildren(d) { count += (d.children || []).length; for (const c of (d.children || [])) countChildren(c) }
  countChildren(dept)
  return count
}
...
const filteredMembers = computed(() => {
  ...
  return activeMembers.value.filter(m => m.username.toLowerCase().includes(kw) || (m.email && m.email.toLowerCase().includes(kw)))
})
...
.dept-sidebar-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: var(--spacing-md); }
.dept-sidebar-header h3 { font-size: var(--font-size-sm); }
.dept-tree { flex: 1; overflow-y: auto; }
...
.dept-stats { display: flex; gap: var(--spacing-md); margin-bottom: var(--spacing-lg); flex-wrap: wrap; }
